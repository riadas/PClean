...
for method in [curr_method] #["synthesis_from_jsons"] # ["synthesis_from_jsons", "direct_synthesis"]
    for benchmark in [curr_benchmark] # ["flights", "hospital", "rents"]
        ...
        results = []
        for filename in [filename] # readdir("$(home_directory)/multiple_hypotheses_base2/hypotheses_copy/$(method)/$(benchmark)")
            filename = split(filename, "/")[end]
            println("compute_overall_best_program: $(benchmark), $(filename)")
            text = ""
            open("$(home_directory)/multiple_hypotheses_base2/hypotheses_copy/$(method)/$(benchmark)/$(filename)") do f 
                text = read(f, String)
            end
            ...
            for i in 1:num_repeats
                ...
                save_file_lines = [
                    ...
                    """CSV.write("$(home_directory)/multiple_hypotheses_base2/results/datasets/$(method)/$(benchmark)/$(replace(filename, ".jl" => "_$(i).csv"))", table)"""
                    ]
                ...
                open("$(home_directory)/multiple_hypotheses_base2/hypotheses_copy/$(method)/$(benchmark)/$(filename)", "w") do f 
                    write(f, text)
                end
                
                include("$(home_directory)/multiple_hypotheses_base2/hypotheses_copy/$(method)/$(benchmark)/$(filename)")
        ...
        if !isdir("$(home_directory)/multiple_hypotheses_base2/results/$(method)")
            mkdir("$(home_directory)/multiple_hypotheses_base2/results/$(method)")
        end
        open("$(home_directory)/multiple_hypotheses_base2/results/$(method)/$(benchmark)_results_overall.txt", "w") do f 
            write(f, join(results, "\n"))
        end
        open("$(home_directory)/multiple_hypotheses_base2/results/$(method)/$(benchmark)_best_program_overall.jl", "w") do f 
