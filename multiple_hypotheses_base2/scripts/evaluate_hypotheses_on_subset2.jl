        ...
        for benchmark in ["flights"] # "rents", "hospital", 
            ...
            for filename in readdir("$(home_directory)/multiple_hypotheses_base2/hypotheses/$(method)/$(benchmark)")
                ...
                if occursin(".jl", filename) 
                    println("read text")
                    open("$(home_directory)/multiple_hypotheses_base2/hypotheses/$(method)/$(benchmark)/$(filename)") do f 
                        text = read(f, String)
                    end
                    ...
                    println("write text")
                    open("$(home_directory)/multiple_hypotheses_base2/hypotheses/$(method)/$(benchmark)/$(filename)", "w") do f 
                        write(f, text)
                    end
                    ...
                    for i in 1:num_repeats
                        println("repeat $(i)") 
                        include("$(home_directory)/multiple_hypotheses_base2/hypotheses/$(method)/$(benchmark)/$(filename)")
            ...
            open("$(home_directory)/multiple_hypotheses_base2/results/$(method)/$(benchmark)_results_subset_$(subset_size).txt", "w") do f 
                write(f, join(results, "\n"))
            end
            open("$(home_directory)/multiple_hypotheses_base2/results/$(method)/$(benchmark)_best_program_$(subset_size).jl", "w") do f 
